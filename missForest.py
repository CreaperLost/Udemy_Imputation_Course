...
class IterativeImputer(_BaseImputer):
    ...
    def __init__(self,parameters: dict, names: list, vmaps: dict,
        ...
        self.min_weight_fraction_leaf = parameters.get("min_weight_fraction_leaf",0.0)
        self.max_features = parameters.get("max_features",'auto')
        self.max_leaf_nodes = parameters.get("max_leaf_nodes",30)
        ...
        self.random_state = parameters.get("random_state",43)
        self.verbose = parameters.get("verbose",0)
        self.warm_start = parameters.get("warm_start",False)
        self.class_weight = parameters.get("class_weight",None)

        self.estimatorRegression = RandomForestRegressor
        self.estimatorClassification = RandomForestClassifier
    ...
    def _initial_imputation(self, X, in_fit=False):
        ...
        X = _BaseImputer._validate_data(self,X, dtype=FLOAT_DTYPES, order="F", reset=in_fit,
    ...
    def fit_transform(self, X, y=None):
        ...
        """self._estimator = clone(self.estimatorRegression(n_estimators=self.n_estimators,
                criterion=self.criterion[0],
                max_depth=self.max_depth,
                min_samples_split=self.min_samples_split,
                min_samples_leaf=self.min_samples_leaf,
